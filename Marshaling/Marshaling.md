...
только на своём экземпляре:
```c#
myNetObject[123] = "Hello";
```
Вопреки ограничению, чтобы создать код на C#, разрешающий данное обращение,
свойство ```Foo``` должно возвращать тип, открывающий доступ к стандартному
индексатору:
```c#
class Program
{
    ...
    class NetObject
    {
        public System.String[] Foo { get; } = new string[150];

        public System.String this[uint index]
        {
            get => Foo[index];
            set => Foo[index] = value;
        }
