...
* [Описание интерфейса класса](https://github.com/)
...
который, расширяет класс ```System.Object```, тогда объект .NET предоставит клиентам
COM доступ ко всем трём интерфейсам классов с именами:
...
экспорте метаданных класса в библиотеку типов COM:
```c#
[ClassInterface(ClassInterfaceType.None)]
public class MammalSuperclass : IExplicit
{
    public void M() { ... }
}
```
*здесь интерфейс ```IExplicit``` явно реализован для доступа клиентов COM к классу

Использование интерфейса класса является допустимым вариантом для клиентов со
сценариями, а также клиентов с поздним связыванием, которые не кэшируют
...
интерфейсе.
Если меняется порядок членов при экспорте класса в библиотеку типов COM, также
меняются и идентификаторы ```DispId```, созданные в интерфейсе класса.

Чтобы избежать нарушений в работе клиентов COM с поздним связыванием при
использовании интерфейса класса, применяется атрибут ```ClassInterfaceAttribute```
со значением ```ClassInterfaceType.AutoDispatch``` (задействован по умолчанию):
```c#
[ClassInterface(ClassInterfaceType.AutoDispatch)]
public class MammalSuperclass : IAnother
{
    public void M() { ... }
}
```
...
не могут кэшировать идентификаторы ```DispId``` во время компиляции.
...
Дополнительно рекомендуется избегать использования сдвоенного интерфейса
(```ClassInterfaceType.AutoDual```) для интерфейса класса, так как он также создаёт
трудности при работе с версиями.

Например, внесение изменений в базовый класс может нарушить работу клиентов COM,
использующих интерфейс производного класса.

Однако, сдвоенные интерфейсы позволяют клиентам COM выполнять раннее и позднее
связывание с членами интерфейсов.
_______________________________________________________________________________
