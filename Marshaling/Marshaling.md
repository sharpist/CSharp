...
Статически типизированный доступ к оболочкам RCW проводят типы взаимодействия
COM – сгенерированные прокси-типы, которые открывают доступ к члену .NET
каждому члену COM.

Типы взаимодействия COM генерируются автоматически на основе выбранной
библиотеки COM и компилируются в сборку взаимодействия COM.
Если COM-объект реализует одновременно несколько интерфейсов, инструмент
импорта библиотек типов генерирует одиночный тип, включающий члены всех
интерфейсов.

Используя метаданные, полученные из библиотеки типов, среда выполнения создает
