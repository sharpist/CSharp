...
* ```out``` – задаёт имя создаваемого файла библиотеки типов.
...
```
* ```tlb``` – создает библиотеку типов из указанной сборки.
...
* ```regfile``` – создает для сборки REG-файл, содержащий необходимые записи реестра,
...
```
regasm.exe myLib.dll /regfile:myTest.reg
```
