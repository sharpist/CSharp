...
[Взаимодействие с COM](https://github.com/)
...
```volatile```, отменяющий их кэширование в регистрах процессора так, чтобы каждое
могло быть изменено несколькими потоками, выполняемыми одновременно.
...
```
Независимо от того, что структура ```SharedData``` находится в неуправляемом
