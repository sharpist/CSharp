...
[Разделяемая память](https://github.com/)
...
Например, в структуре, включающей одно поле типа ```int``` и ещё одоно поле типа ```char```,
```int```, идущее первым, размещается на смещении 0 и занимает 4 байта, тогда как ```char```
начнётся со смещения 4 и займёт 2 байта:
```c#
[StructLayout(LayoutKind.Sequential, Pack = 2)]
...
```
Если поле типа ```char``` начнётся со смещения 2, присваивание значения изменит также
и поле типа ```int``` – структура "объединение" доступная в C.
Значение ```LayoutKind.Explicit``` и атрибут ```FieldOffset``` позволяют воспроизвести
объединение в C#.
...
```midiOutShortMsg``` имеет следующую сигнатуру:
```c
