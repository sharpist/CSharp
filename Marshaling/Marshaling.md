...
Таким образом, тип можно объявить без его интерфейса:
```c#
public class Foo
{
...
```
Потребители этого типа вызывают метод напрямую.
Модификация реализации метода и добавление перегруженных версий не нарушает
...
реализацию классом ```Foo``` другого интерфейса – ```IFoo2```, по следующим причинам:
...
Создание версий библиотек COM укладывается в множественное поддержание
интерфейсов.
