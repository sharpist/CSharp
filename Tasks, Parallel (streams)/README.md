...
***1.*** **Чтобы отменить процесс необходимо:**
...
Ниже показан пример, повторяющий описанные принципы:
...
> Класс **```CancellationTokenSource```** реализует интерфейс **```IDisposable```**.
> После завершения использования источника токена отмены требуется безусловное выполнение
метода **```Dispose```**, чтобы освободить все занятые неуправляемые ресурсы.

***2.*** Для отмены выполнения параллельных операций, запущенных с помощью методов **```For```** и **```ForEach```**,
