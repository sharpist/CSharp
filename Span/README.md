...
```Span<T>``` включён в пакет ```System.Memory``` и определяется в пространстве имён ```System```.
...
В отличие от массивов, он может указывать на управляемую или нативную (неуправляемую)
память, или на память, выделенную в стеке:
```c#
// managed memory
...
#### Дополнение относительно языка C# и компилятора: ####

Прежде уже упоминалось о функционале, добавленном в язык C# и компилятор, чтобы помочь
сделать ```Span<T>``` первоклассным участником в .NET. Несколько особенностей C# 7.2 связаны
...
Ключевое слово ref перед объявлением struct, указывает компилятору C# разрешить
использование других ```ref struct``` типов, подобных ```Span<T>``` в качестве полей, и при этом
также регистрировать вызванные ограничивающие условия, которые будут назначены взятому
типу.

Например, если бы требовалось создать структуру перечислитель Enumerator для ```Span<T>```,
этому перечислителю необходимо было бы хранить ```Span<T>``` и, таким образом, он сам должен
был бы являться ```ref struct```:
```c#
...
### ```Stackalloc``` инициализация диапазонов spans ###

В предыдущих версиях C# результат ```stackalloc``` мог быть сохранен только в локальной
...
```
Допускается просто написать:
```c#
Span<byte> bytes = stackalloc byte[length];
```
Это также чрезвычайно полезно в ситуациях, когда для выполнения операции требуется
некоторое свободное пространство, но необходимо избежать выделения памяти кучи для
относительно небольших размеров.

Раньше имелось два варианта:

* создать двое, совершенно различных друг от друга, путей выполнения кода, выделяющих и
оперирующих над памятью на основе стека и кучи.

* закрепить память, связанную с управляемым выделением и после делегировать к реализации,
также используемой для памяти на основе стека, и записавыть с управлением указателя в
небезопасном коде.

Теперь то же самое может быть выполнено без дублирования кода, совместно с безопасным
кодом и с минимальными затратами:
```c#
Span<byte> bytes = length <= 128 ? stackalloc byte[length] : new byte[length];
... // Code that operates on the Span<byte>
```

### ```Span``` проверка на допустимость использования ###

Поскольку диапазоны spans могут ссылаться на данные, которые могли бы быть
ассоциированными с заданным стековым кадром (фреймом), может быть опасно передавать
spans таким образом, чтобы они могли давать возможность ссылаться на память, которая
более не действительна.

Например, этот метод, который пытается сделать следующее:
```c#
...
```
Здесь пространство было выделено из стека, и затем предпринимается попытка возврата
ссылки на это пространство, но с момента возврата, это пространство больше не будет
являться действительным для использования.
