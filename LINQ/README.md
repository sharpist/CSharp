...
[Группирование](https://github.com/sharpist/C_Sharp/tree/master/LINQ#группирование)

[Операции OfType() и Cast()](https://github.com/sharpist/C_Sharp/tree/master/LINQ#операции-oftype-и-cast)
...
*/
```
Для сортировки добавляется после конструкции group продолжение ```into``` с новой
...
Необобщённые коллекции ```IEnumerable``` приводятся к обобщённой последовательности
```IEnumerable<T>``` операциями ```OfType``` и ```Cast```.
Если входной элемент с несовместимым типом:
* ```Cast``` – генерирует исключение.
* ```OfType``` – пропускает.

Тип целевого элемента подставляется непосредственно после генератора ```from``` в
запросе:
```c#
...
