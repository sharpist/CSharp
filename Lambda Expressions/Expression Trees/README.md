...
1. Методы фабрики в пространстве имён ```System.Linq.Expressions``` используются для создания
деревьев выражений, представляющих общий запрос.

2. Выражения, представляющие вызовы методов стандартных операторов запросов, ссылаются на
реализации ```Queryable``` классом группы статических методов расширения, которые расширяют
любой объект, реализующий ```IQueryable<T>``` интерфейс.
...
В примере использовано фиксированное число выражений в предикате, передаваемом в метод
```Queryable.Where```.
Можно применять переменное число выражений предиката, зависящих от вводимых данных
и изменять стандартные операторы запросов.
