...
Когда не требуется определённая форма взаимодействия ```Order``` можно не
указывать. При упорядочении по алфавиту расхождение между сериализацией и
десериализацией при добавлении и удалении членов не произойдёт (за исключением
перемещения члена между базовым классом и подклассом).
