...
// 32768
```
“ак как задаютс€ типы указателей, ```stackalloc``` требует небезопасного контекста.
...
Ёта пам€ть не подвергаютс€ сборке мусора и еЄ не нужно закрепл€ть с помощью атрибута
```fixed```.
