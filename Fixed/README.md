...
Оператор ```fixed``` задает указатель ([Типы указателей](https://github.com/sharpist/C_Sharp/tree/master/Pointer#Типы-указателей)) на управляемую переменную и "закрепляет" эту переменную
