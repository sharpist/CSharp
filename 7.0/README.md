...
```
Отдельные поля можно извлекать, назначая кортежу класс,
предоставляющий этот метод:
```c#
var p = new Point(3.14, 2.71);
(double X, double Y) = p;
```
Допустимы перегруженные версии методов Deconstruct с различными out
параметрами:
```c#
...
```
При этом, определённому объявлению кортежа всегда сопоставляется
соответствующая перегрузка:
```c#
...
Функция возвращает ссылку на переменную ([Возвращаемые ссылочные значения](https://github.com/sharpist/C_Sharp/tree/master/Ref%20returns#Возвращаемые-ссылочные-значения-и-ссылочные-локальные-переменные)) – элемент матрицы, который
